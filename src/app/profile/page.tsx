...
import { Activity, Award, Clock, Target, Trophy, User } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  if (!userData) {
    return <p>Loading profile...</p>; // Show a loading message while data is being fetched
  }
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold">{(user?.avgSpeed || 0).toFixed(2)} WPM</span>
          </div>
        </div>
      </CardContent>
    </Card>
          ...
          {history.slice(0, 5).map((test) => (
            <div key={test._id} className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Accuracy: {test.accuracy}%</p>
              </div>
